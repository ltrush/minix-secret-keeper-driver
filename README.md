...
Working inside the Minix driver framework meant operating below the filesystem, which forced me to think carefully about things that higher-level code abstracts away. Buffer addresses passed in from user processes are virtual addresses in their address space, not the driver's, so every byte transfer has to be mediated by the kernel. The secret is also treated as raw bytes throughout since there is no guarantee the contents are null-terminated or printable.

The trickiest part was getting the reset condition right: the secret should persist after the writer closes, but once any read file descriptor has been opened, the secret wipes itself when the last fd closes. That required tracking an open count and whether a read fd had ever been opened during the current ownership cycle.
